...
{
    [Obsolete]
    public class MainAccounRegister_obsolete : ServicesDataAndComponents_obsolete
    {
        ...
        /// </summary>
        public MainAccounRegister_obsolete()
        {
        ...
        public void Start()
        {
            ...
            if (_beforeRegistrationGoToWalkingZen)
            {
                ...
                if (numbOfWalks == 0 || numbOfWalks != 0 && !_skipWalkingOnZenIfWalked)
                {
                    ...
                    new MainWalkerOnZen_obsolete(ObjectTypeEnum.Donor).Start();

                    // Проверка прогулки по yandex.zen (если статус false - разгружаем ресурсы и завершаем работу скрипта)
                    if (!MainWalkerOnZen_obsolete.StatusWalkIsGood) return;
                }
            }
        ...
        private bool ResourceHandler()
        {
            lock (SyncObjects.TableSyncer)
            {
                ...
                for (int row = 0; row < accountsCount; row++)
                {
                    ...
                    DataKeeper_obsolete.ResourcesCurrentThread.Add(InstagramUrl);
                    DataKeeper_obsolete.ResourcesAllThreadsInWork.Add(InstagramUrl);
